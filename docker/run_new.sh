...
if [ $SERVER -eq 1 ]; then
        ...
        --net=host \
        --volume "${CONFIG_FILE_COMM}:${CATKIN_WS}/src/covins/covins_comm/config/config_comm.yaml" \
        --volume "${LAUNCH_FILE}:${CATKIN_WS}/src/covins/orb_slam3/Examples/ROS/ORB_SLAM3/launch/launch_docker_ros_gazebo_agent1.launch" \
        --volume "/home/shirb/ws/covins_ws/src/covins/orb_slam3/Examples/real_camera_hector.yaml:${CATKIN_WS}/src/covins/orb_slam3/Examples/real_camera_hector.yaml" \
        covins \
        /bin/bash \
       # /bin/bash -c \
       #         "cd ${CATKIN_WS}; \
